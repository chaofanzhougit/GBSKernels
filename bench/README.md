...
Every harness shares the same discipline: one input generator (so cross-engine
timings are same-input), randomized execution order, raw repeats reported as
...
so an asynchronous early return cannot fake a fast time.
