// check_permanent_warp.cu -- on-device gate for the FUSED warp permanent.
//
// GPU-ONLY: permanent_warp.cu uses __shfl_down_sync, which the CPU host shim cannot
// emulate, so this gate is NOT in run_preflight.sh. It is built by nvcc and run in a
// rented-GPU session (scripts/gpu_session.sh / core/CMakeLists.txt). Validates the
// fused warp permanent against an independent host Glynn (the reference
// check_permanent.cu uses, itself validated vs combinatorics + The Walrus on CPU).
//
//   nvcc -O3 -std=c++17 permanent_warp.cu check_permanent_warp.cu -o check_permanent_warp
...
namespace gbs {
extern "C" int gbs_perm_glynn_warp_batched(const cuDoubleComplex*, int, int,
                                           cuDoubleComplex*, cudaStream_t);
}

// Independent host Glynn permanent (identical to check_permanent.cu / _coop).
...
int main() {
  ...
  for (int n = 1; n <= 14; ++n) {
    ...
    for (int b = 0; b < batch; ++b) ref[b] = host_perm_glynn(host.data() + (size_t)b * n * n, n);
    ...
    gbs::gbs_perm_glynn_warp_batched(d_in, n, batch, d_out, 0);
  ...
  std::printf("max relative error (n=1..14, complex, warp-shuffle): %.3e  [tol %.1e]\n", worst, tol);
  std::printf("%s\n", worst < tol ? "PASS" : "FAIL");
